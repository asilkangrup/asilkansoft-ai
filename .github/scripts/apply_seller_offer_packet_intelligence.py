...
# 1) Persist offer-packet intelligence automatically whenever an isolated profile changes.
replace_once(
...
# 2) Inject the privacy-safe seller offer packet summary into every isolated chat turn.
replace_once(
...
# 3) Classify media explicitly, so actual property photos are not confused with tapu/listing screenshots.
replace_once(
...
# 4) Make the next-best-action orchestrator complete the investor-offer packet without blocking valuation on photos.
replace_once(
...
# 5) Seller negotiation strategy must move toward transparent investor offers, not seller price-maximization coaching.
replace_once(
...
# 6) Reuse the structured offer-packet intelligence in the deterministic high-price reply.
p = Path('app/Services/RealEstateWhatsAppInboundService.php')
s = p.read_text()
old = """        $data = is_array($profile->data) ? $profile->data : [];
        $missing = [];
...
"""
if old not in s:
    raise SystemExit('seller steering block not found')
p.write_text(s.replace(old, new, 1))

# The patch helper and one-shot workflow are removed from the resulting feature commit.
Path('.github/scripts/apply_seller_offer_packet_intelligence.py').unlink(missing_ok=True)
Path('.github/workflows/apply-seller-offer-packet-intelligence.yml').unlink(missing_ok=True)
