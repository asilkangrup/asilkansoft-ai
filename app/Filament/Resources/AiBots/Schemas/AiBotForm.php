...
class AiBotForm
{
        ...
        return $schema
            ->components([
                ...
                Section::make('Yapay Zekâ Yönetimi')
                    ->description('Yapay zekânızın çalışma durumunu, WhatsApp bağlantısını ve paket bilgilerini tek yerden yönetin.')
                    ->icon('heroicon-o-cpu-chip')
                    ->schema([
                        Toggle::make('ai_enabled')
                            ->label('Yapay Zekâ Aktif')
                            ->helperText('Kapattığınızda WhatsApp bağlantısı ve QR kodu bağlı kalır, ancak yapay zekâ müşterilere otomatik cevap vermez.')
                            ->default(true)
                            ->onColor('success')
                            ->offColor('danger')
                            ->live(),
                        ...
                        Placeholder::make('whatsapp_connection_status')
                            ...
                            ->content(fn (?AiBot $record): string => match ((string) ($record?->whatsapp_status ?? '')) {
                                'connected' => '🟢 Bağlı',
                        ...
                        Placeholder::make('subscription_status_display')
                            ->label('Paket Durumu')
                            ->content(fn (?AiBot $record): string => match ((string) ($record?->subscription_status ?? '')) {
                                'active' => '🟢 Aktif',
                        ...
                        Placeholder::make('message_usage_display')
                            ...
                            ->content(function (?AiBot $record): string {
                                if (! $record) {
                                    return '-';
                                }
                                ...
                                if ($record->subscription_status === 'trial') {
                                    ...
                                    return $used.' / '.$limit.' kullanıldı — '.$remaining.' mesaj kaldı';
                                }
                ...
                Section::make('WhatsApp Grup Yönlendirme')
                    ...
                    ->visible(fn (?AiBot $record): bool => (int) ($record?->id ?? 0) === 30)
                    ->collapsed()
                    ->schema([
                        Toggle::make('group_routing_enabled')
                            ->label('Grup Yönlendirmeyi Aktif Et')
                            ->helperText('Açıldığında tamamlanan başvurular aşağıda seçtiğiniz WhatsApp gruplarına otomatik gönderilir.')
                            ->default(false)
                            ->onColor('success')
                            ->offColor('danger')
                            ->live(),
                        ...
                        Select::make('vodafone_group_jid')
                            ...
                            ->options(fn (?AiBot $record): array => self::whatsAppGruplari($record))
                            ->searchable()
                            ->native(false)
                            ->visible(fn ($get): bool => (bool) $get('group_routing_enabled')),
                        ...
                        Select::make('findeks_group_jid')
                            ...
                            ->placeholder('WhatsApp grubu seçin')
                            ->helperText('Findeks / banka kredi danışmanlığı başvuruları bu gruba gönderilir.')
                            ->options(fn (?AiBot $record): array => self::whatsAppGruplari($record))
                            ->searchable()
                            ->native(false)
                            ->visible(fn ($get): bool => (bool) $get('group_routing_enabled')),
                        ...
                        Select::make('elden_taksit_group_jid')
                            ...
                            ->placeholder('WhatsApp grubu seçin')
                            ->helperText('Bankasız / kefilsiz elden taksit başvuruları bu gruba gönderilir.')
                            ->options(fn (?AiBot $record): array => self::whatsAppGruplari($record))
                            ->searchable()
                            ->native(false)
                            ->visible(fn ($get): bool => (bool) $get('group_routing_enabled')),
                ...
                Section::make('Temel Bilgiler')
                    ...
                    ->schema([
                        ...
                        Select::make('business_sector')
                            ...
                            ->helperText('Sektör değiştiğinde WAI lead puanlama profilini otomatik olarak yeniden ayarlar.')
                ...
                Section::make('Firma Bilgileri')
                    ...
                    ->schema([
                        Textarea::make('company_description')
                            ...
                            ->helperText('Yapay zekâ müşterilere firmanızı anlatırken bu bilgileri kullanır.')
                            ->rows(7)
                            ->required()
                            ->columnSpanFull(),
                ...
                Section::make('Yapay Zekâ Eğitimi')
                    ...
                    ->schema([
                        Textarea::make('company_rules')
                            ...
                            ->helperText('Yapay zekânın kesinlikle uyması gereken firma kurallarını yazın.')
                            ->rows(7)
                            ->columnSpanFull(),
    ...
    private static function whatsAppGruplari(?AiBot $record): array
    {
        if (! $record) {
            return [];
        }

        $instanceName = trim((string) $record->whatsapp_instance);
        ...
        try {
            $groups = app(WhatsAppService::class)->fetchGroups($instanceName);
            $options = [];
