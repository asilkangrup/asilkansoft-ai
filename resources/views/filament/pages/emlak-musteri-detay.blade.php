...
.ed-gallery-card{grid-column:1/-1}.ed-gallery-groups{display:grid;gap:15px}.ed-gallery-group h3{margin:0 0 8px;font-size:11px;color:#536159}.ed-gallery{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.ed-media{display:block;overflow:hidden;border:1px solid var(--line);border-radius:13px;background:#f7faf8;color:#315b9c;text-decoration:none}.ed-media img{display:block;width:100%;aspect-ratio:4/3;object-fit:cover}.ed-media-file{display:grid;place-items:center;aspect-ratio:4/3;padding:12px;text-align:center;font-size:11px;font-weight:900}.ed-media-meta{display:block;padding:7px;font-size:9px;color:var(--muted)}.ed-timeline{display:grid;gap:8px}.ed-event{padding:11px;border-radius:12px;background:#f7faf8}.ed-event b{font-size:11px;color:#35463d}.ed-event p{margin:4px 0;font-size:11px;line-height:1.45;color:#536159}.ed-event small{font-size:9px;color:var(--muted)}
@media(max-width:720px){.ed-gallery{grid-template-columns:repeat(2,minmax(0,1fr))}.ed-grid{grid-template-columns:1fr}.ed-name{font-size:22px}.ed-card{padding:15px}.ed-noteform{grid-template-columns:1fr}.ed-noteform textarea{font-size:16px}.ed-noteform button{height:44px}.ed-call{width:100%;justify-content:center}}
</style>
<div class="ed">
    ...
    <div class="ed-grid">
        ...
        @if(count($this->mediaGallery))
        <section class="ed-card ed-gallery-card">
            <h2>Taşınmaz Görselleri ve Belgeleri</h2>
            <div class="ed-gallery-groups">
                @foreach($this->mediaGallery as $group => $items)
                    <div class="ed-gallery-group">
                        <h3>{{ mb_strtoupper($group) }} · {{ count($items) }}</h3>
                        <div class="ed-gallery">
                            @foreach($items as $media)
                                <a class="ed-media" href="{{ $media['url'] }}" target="_blank" rel="noopener noreferrer">
                                    @if($media['is_image'])
                                        <img src="{{ $media['url'] }}" alt="{{ $group }}" loading="lazy">
                                    @else
                                        <span class="ed-media-file">📄 Belgeyi aç</span>
                                    @endif
