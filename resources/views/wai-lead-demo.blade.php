...
<head>
    ...
    <style>
        :root{--bg:#07110d;--panel:#0d1713;--line:#1d2a24;--text:#f5f7f6;--muted:#8fa097;--green:#25d366;--mine:#005c4b;--theirs:#202c33;--amber:#f4c15d}
        *{box-sizing:border-box}html,body{margin:0;min-height:100%;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Inter,sans-serif;background:radial-gradient(circle at top,#153227 0,#07110d 42%,#050806 100%);color:var(--text)}
        ...
        .sheetback{position:fixed;inset:0;background:#000b;z-index:20;display:flex;align-items:flex-end;justify-content:center}.sheet{width:min(100%,520px);background:#111b17;border-radius:24px 24px 0 0;padding:20px 18px calc(24px + env(safe-area-inset-bottom));border:1px solid #26362e;box-shadow:0 -20px 60px #000a}.grab{width:42px;height:4px;border-radius:99px;background:#42544b;margin:-8px auto 18px}.sheet h2{font-size:20px;margin:0 0 7px}.sheet p{font-size:13px;color:#9eaea5;line-height:1.5;margin:0 0 12px}.qrbox{width:238px;height:238px;margin:12px auto;background:#fff;border-radius:18px;display:grid;place-items:center;padding:12px}.qrbox img{width:100%;height:100%;object-fit:contain}.status{padding:10px;border-radius:12px;background:#17221d;text-align:center;font-size:12px;color:#b7c6be}.warning{margin:10px 0;padding:11px 12px;border-radius:12px;background:#2a2314;border:1px solid #4b3b1b;color:#f5d88d;font-size:12px;line-height:1.45}.devicehint{margin:10px 0;padding:11px 12px;border-radius:12px;background:#14211b;border:1px solid #263b31;color:#b9c9c0;font-size:12px;line-height:1.45}.actions{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px}.action{padding:11px;border-radius:12px;border:1px solid #314138;background:#17251e;color:#e1ebe6;font-weight:800;cursor:pointer}.action.primary{background:#1d5d38;border-color:#2f7d4d;color:#d8ffe5}.close{width:100%;margin-top:10px;padding:11px;border-radius:12px;border:1px solid #314138;background:transparent;color:#dce5e0;font-weight:700}.connected{color:#77ee9f;font-weight:800}
...
<div class="shell">
    ...
    <main id="chat" class="chat">
        ...
        <div class="row ai"><div class="bubble">Merhaba 👋 {{ $lead['company_name'] ?? 'İşletmemiz' }}'e hoş geldiniz. Size en doğru şekilde yardımcı olabilmem için talebinizi kısaca paylaşır mısınız?<span class="time">şimdi</span></div></div>
    </main>

    <div class="bottom">
        <div class="composer">
            <div class="inputwrap"><textarea id="message" maxlength="500" rows="1" placeholder="Müşterinizmiş gibi yazın"></textarea></div>
...
<div id="sheet" class="sheetback hidden">
    <section class="sheet">
        ...
        <div class="devicehint"><b>Bu sayfayı WhatsApp'ı bağlayacağınız telefonda açtıysanız:</b> QR kodunu aynı telefondan okutamazsınız. Aşağıdaki “Linki kopyala” butonuna basın, bağlantıyı başka bir telefon veya bilgisayarda açın ve QR kodunu bu telefonunuzdaki WhatsApp ile okutun.</div>
        <div id="qrbox" class="qrbox"><div class="typing"><i></i><i></i><i></i></div></div>
        <div id="waStatus" class="status">QR hazırlanıyor…</div>
        <div id="qrWarning" class="warning hidden">QR kodunu 30 saniye içinde okutmadıysanız güvenlik nedeniyle yenileyin. <span id="countdown"></span></div>
        <div class="actions">
            <button id="refreshQr" class="action primary" type="button">QR'ı yenile</button>
            <button id="copyLink" class="action" type="button">Linki kopyala</button>
        </div>
...
function showConnected(){clearInterval(poll);clearInterval(qrTimer);qrWarning.classList.add('hidden');qrbox.innerHTML='<div style="font-size:58px">✓</div>';waStatus.innerHTML='<span class="connected">WhatsApp bağlandı. 1 günlük denemeniz başladı.</span>';connect.textContent='WhatsApp bağlı ✓'}
async function copyCurrentLink(){try{await navigator.clipboard.writeText(window.location.href);copyLink.textContent='Kopyalandı ✓';setTimeout(()=>copyLink.textContent='Linki kopyala',1800)}catch(e){const t=document.createElement('textarea');t.value=window.location.href;document.body.appendChild(t);t.select();document.execCommand('copy');t.remove();copyLink.textContent='Kopyalandı ✓';setTimeout(()=>copyLink.textContent='Linki kopyala',1800)}}
send.addEventListener('click',submit);input.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();submit()}});input.addEventListener('input',()=>{input.style.height='45px';input.style.height=Math.min(input.scrollHeight,110)+'px'});connect.addEventListener('click',connectWA);refreshQr.addEventListener('click',connectWA);copyLink.addEventListener('click',copyCurrentLink);document.getElementById('closeSheet').addEventListener('click',()=>sheet.classList.add('hidden'));
</script>
