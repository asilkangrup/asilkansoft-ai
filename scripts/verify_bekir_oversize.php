...
foreach(['Polo Yaka Tişört','Sıfır Yaka Tişört','Premium Oversize Tişört'] as $product){
...
Cache::store('database')->put($key,['product'=>'Premium Oversize Tişört','product_category'=>'shirt','color'=>'white','color_label'=>'Beyaz','quantity'=>50,'logo_received'=>true,'logo_base64'=>$b64,'logo_mime'=>'image/png','position'=>null,'mockup_sent'=>false],now()->addMinutes(5));
$before=Http::recorded()->count();
$service->processPayload(['instance'=>$bot->whatsapp_instance,'event'=>'messages.upsert','data'=>['key'=>['remoteJid'=>$phone.'@s.whatsapp.net','id'=>'SIDES_QA_'.bin2hex(random_bytes(4)),'fromMe'=>false],'message'=>['conversation'=>'oversize '.$text]]],true);
$state=Cache::store('database')->get($key);
ck(($state['product']??null)==='Premium Oversize Tişört','oversize accepted');
